...
namespace waverider {
ParallelizedPolicy2D::ParallelizedPolicy2D(uint num_policies, PolicyTuning tuning)
    : tuning_(tuning), num_policies_(num_policies) {}

void ParallelizedPolicy2D::init(const std::vector<Eigen::Vector3f>& x_obs,
  ...
  for (uint i = 0; i < num_policies_; ++i) {
    ...
    // eq 68 in RMP paper
    const Eigen::Vector2f P_obs_x = std::max(0.0f, -xdot.dot(grad_d)) *
                                    (grad_d * grad_d.transpose()) * (xdot);
    const Eigen::Vector2f f_damp = alpha_damp * P_obs_x;

    // set f
    Eigen::Vector2f f_temp{Eigen::Vector2f::Zero()};
    ...
    const Eigen::Vector2f v = s(f_temp);
    Eigen::Matrix2f A_temp =
